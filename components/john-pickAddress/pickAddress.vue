<template>
  ...
  <view class="picker-box" :style="{ height: screenHeight + 'px' }">
    <view class="picker-close iconfont icon-guanbi" @click="onClose()"> </view>
    <view class="box-select-button">
      <view
        class="button"
        :class="{ 'button-select': tabIndex === 0 }"
        @click="onTab(0)"
      >
        {{ provinceName }}
      </view>
      <view
        class="button"
        :class="{ 'button-select': tabIndex === 1 }"
        @click="onTab(1)"
      >
        {{ cityName }}
      </view>
      <view
        class="button"
        :class="{ 'button-select': tabIndex === 2 }"
        @click="onTab(2)"
      >
        {{ districtName }}
      </view>
...
export default {
  ...
  methods: {
    ...
    onSelectConfirm() {
      console.log(
        `省：${this.province.name},市：${this.city.name}, 区：${this.district.name}`
      );
      console.log(
        `省code：${this.province.id},市code：${this.city.id}, 区code：${this.district.id}`
      );
      let v =
        this.province.name + " " + this.city.name + " " + this.district.name;
      this.$emit("onSelectConfirm", v);
      // this.$refs.popup.close()
    },
  },
};
